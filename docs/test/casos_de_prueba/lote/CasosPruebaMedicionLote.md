...
`estado` es un criterio de negocio legítimo para el usuario (a diferencia de una baja lógica, acá "ver lo anulado" tiene valor real) y no asume `REGISTRADO` por defecto: `null` muestra mediciones en cualquier estado. El rango de fechas (`fechaMedicionDesde`/`fechaMedicionHasta`) sigue la regla habitual: nulo no acota.
...
|**CP-FML-02**|Estado nulo no asume REGISTRADO por defecto; se propaga tal cual junto con el rango de fechas nulo sin acotar|`idEtapaLote: 1L`, `idDetalleParametroControl: 5L`, `estado: null`, `fechaMedicionDesde/Hasta: null`, `pageable: PageRequest.of(0, 10)`, BD con 1 medición REGISTRADO y 1 ANULADO|El service propaga los 3 parámetros nulos tal cual al repositorio|Retorna `Page<MedicionLoteResponseDTO>` con las 2 mediciones, sin excluir la ANULADA.|
