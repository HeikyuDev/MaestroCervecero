...
|**CP-IL-17**|Mismo lúpulo en dos detalles (WHIRLPOOL + DRY_HOP) se suma|1 lúpulo usado en 2 `DetalleLupuloEntity` (10g WHIRLPOOL + 5g DRY_HOP, volumen objetivo = volumen base)|`requerimientosPorInsumo.merge(...)` por el mismo ID de insumo|El requerimiento se suma (15.0g), generando una única reserva de 15.0g — no se pisan ni se duplican.|
