...
|---|---|---|---|---|
|**CP-BT-01**|Consulta con registros existentes|`pageable: PageRequest.of(0, 10)`, BD con 2 ollas activas|`findAll(pageable)` contiene elementos|Retorna `Page<OllaHervorResponseDTO>` con 2 elementos mapeados.|
|**CP-BT-02**|Consulta sin registros existentes|`pageable: PageRequest.of(0, 10)`, BD vacía|`findAll(pageable)` está vacío|Retorna `Page<OllaHervorResponseDTO>` vacía (`getContent().isEmpty() == true`).|
...
| **CP-AO-08** | Alta exitosa _(Camino feliz)_              | `capTotal: 100.0`, `capUtil: 80.0`, `evaporacion: 10.0`, `trub: 3.0`, `idInterno: "OLLA-02"` (Único) | Todas las validaciones $\rightarrow$ **FALSE**                   | Persiste la entidad con `estadoOperativo = DISPONIBLE` y retorna DTO.      |
...
|**ID**|**Nombre del Caso**|**Datos de Entrada (Escenario)**|**Condición Evaluada**|**Resultado Esperado**|
|---|---|---|---|---|
|**CP-BO-01**|Baja de olla inexistente|`id: 99L` (No existe en BD)|`findById(99L)` $\rightarrow$ **Optional.empty()**|Lanza `RecursoNoEncontradoException`. No llama a `delete()`.|
|**CP-BO-02**|Baja exitosa _(Soft Delete)_|`id: 1L` (Existe en BD)|`findById(1L)` $\rightarrow$ **Presente**|Invoca `delete(entity)` y retorna DTO de la olla dada de baja.|