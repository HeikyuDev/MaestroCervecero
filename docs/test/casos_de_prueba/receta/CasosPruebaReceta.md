...
|**CP-BR-02**|Baja exitosa _(Baja lógica vía Estado)_|`id: 1L` (Existe en BD, sin planificaciones de producción pendientes asociadas)|`existsByVersionReceta_Receta_IdAndEstado(1L, PENDIENTE)` $\rightarrow$ **FALSE**|Setea `estado = BAJA` en la entidad, invoca `save(entity)` y retorna DTO de la receta dada de baja.|
|**CP-BR-03**|Baja rechazada por planificación de producción pendiente asociada|`id: 1L` (Existe en BD); existe una planificación de producción en estado `PENDIENTE` asociada a alguna versión (histórica o activa) de la receta|`existsByVersionReceta_Receta_IdAndEstado(1L, PENDIENTE)` $\rightarrow$ **TRUE**|Lanza `ReglaNegocioException`. No llama a `save()`.|
|**CP-BR-04**|Baja permitida con planificaciones asociadas en otro estado|`id: 1L` (Existe en BD); tiene planificaciones `FINALIZADA`/`ANULADA` asociadas, pero ninguna `PENDIENTE`|`existsByVersionReceta_Receta_IdAndEstado(1L, PENDIENTE)` $\rightarrow$ **FALSE**|Setea `estado = BAJA` en la entidad, invoca `save(entity)` y retorna DTO de la receta dada de baja.|